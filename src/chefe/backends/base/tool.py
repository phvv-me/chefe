...
class Tool:
    ...
    command to the workspace) and ``available`` (a guard). Building the argv is this class's job
    and spawning the child is :class:`Process`'s, so a backend only ever describes what to run.

    Reporting what an ecosystem has provisioned is deliberately not declared here. Only the three
    concrete backends answer it, each against a different source of truth, and nothing calls it
    through this base, so stating it would force every future backend to carry a method it does
    not implement.
    """
    ...
    def command(self) -> BaseCommand:
        ...
        A backend that runs through another tool (cargo through `pixi run`) overrides
        :meth:`__call__` and never names a binary, so the name is required here, at the one
        boundary that needs it, rather than of every subclass.
        """
        if not self.name:
            raise ChefeError(f"{type(self).__name__} names no command of its own to run")
