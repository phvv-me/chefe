...
class Writer:
    ...
    A caller cannot build one of these, only receive it from `GeneratedFiles.locked()`, and every
    edit re-checks that lock, so an instance stashed past its block fails loudly instead of
    racing the process that holds the lock now.
    """
