...
from chefe.workspace.generated import ActivationScript, module_init_snippet

_MODULES = ("nvidia/26.3", "gcc/15.2.0")


def render(tmp_path: Path, modules: tuple[str, ...], hook: str = "export FOO=bar") -> str:
    """The `activate.sh` text an :class:`ActivationScript` writes for ``modules``."""
    return ActivationScript(tmp_path / "activate.sh", hook).render(modules)


def test_module_script_contents(tmp_path: Path) -> None:
    """With modules pinned the script sources module init and loads exactly the pinned list.

    Every module command sits behind `command -v module`, the script purges first, and it
    carries no libstdc++ hack.
    """
    script = render(tmp_path, _MODULES)
    present = [
        "module purge",
...
def test_empty_modules_emit_no_module_block(tmp_path: Path) -> None:
    """With no modules configured the whole module block is omitted.

    Sourcing the script then never purges whatever stack the surrounding job had loaded.
    """
...
def test_write_persists_the_script(tmp_path: Path) -> None:
    ...
    path = ActivationScript(tmp_path / "activate.sh", "export FOO=bar").write(_MODULES)
...
def test_activation_relaxes_nounset_around_sourcing(tmp_path: Path) -> None:
    ...
    script = render(tmp_path, _MODULES, hook='echo "hook ran: ${UNDEFINED_VAR:-}"')
