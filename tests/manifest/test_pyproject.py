...
from chefe.workspace import Workspace

# The interpreter is declared under `[tool.chefe.conda]` because Python has no PEP 508 form.
_PYPROJECT = """
[project]
name = "demo"
version = "9.9.9"
...
def write_pyproject(root: Path, text: str = _PYPROJECT) -> Path:
...
def test_manifest_body_splits_deps_across_conda_and_pypi() -> None:
    ...
    manifest = Manifest.from_pyproject(_PYPROJECT)
...
def test_manifest_body_reads_the_dev_group_with_includes() -> None:
    ...
    dev = Manifest.from_pyproject(_PYPROJECT).dev.toolchains()["python"].deps
...
def test_manifest_body_self_installs_the_package_editable() -> None:
    ...
    demo = (Manifest.from_pyproject(_PYPROJECT).python().deps["demo"].model_extra) or {}
...
"""
    ...
    python = body["python"]
    assert python["dependency-overrides"] == {"sqlalchemy": ">=2.1.0b3"}
...
def test_discover_walks_up_to_a_pyproject_workspace(tmp_path: Path) -> None:
    ...
    assert Workspace.discover(nested) == tmp_path


@pytest.mark.parametrize(
    "mutate",
    [
        lambda manager: manager.dependencies.add("requests", language="conda"),
        lambda manager: manager.dependencies.remove("httpx"),
        lambda manager: manager.dependencies.upgrade("httpx"),
        lambda manager: manager.dependencies.upgrade(),
    ],
