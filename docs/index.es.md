...
Conda, PyPI, npm, cargo. Los proyectos reales necesitan varios a la vez, dispersos entre `pixi.toml`, `package.json` y `Cargo.toml`. chefe es el jefe de cocina. Tú escribes **una receta `chefe.toml`**, esta compila cada manifest nativo bajo `.chefe/`, ejecuta las herramientas reales y los sirve como un único entorno. Nunca vuelve a implementar un solucionador. Dirige a los cocineros.
...
- :material-cog-transfer-outline: **Salida nativa**
    ...
    Compila a `pixi.toml`, `package.json` reales y sus amigos. Las herramientas reales hacen la resolución.
...
flowchart TB
    ...
    PY --> PT
    NP --> PJ
    CG -. sin archivo, instala in-place .-> CARGO
...
- `pixi` (con `uv` dentro) es el motor profundo para conda y PyPI, y los otros ecosistemas son capas delgadas y explícitas por encima.
