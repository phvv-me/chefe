...
実際の `chefe.toml` を見ることが、最も説得力のあるツアーになります。これは、マルチ言語の ML monorepo の背後にある manifest です。読みやすさのために調整されていますが、すべての機能が網羅されています。conda + PyPI (カスタムインデックスからの torch) + npm + cargo、システム CUDA の基盤、環境変数、プラットフォームオーバーレイ、分離された serving 環境、そしてタスクが含まれています。
...
chefe shell                # すべてのバイナリが PATH に通ったアクティブ化されたシェル
