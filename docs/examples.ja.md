...
実際の `chefe.toml` は、最も説得力のあるツアーです。これは、マルチ言語の ML monorepo の背後にある manifest であり、読みやすさのために調整されていますが、すべての機能が網羅されています。conda + PyPI (カスタムインデックスからの torch) + npm + cargo、システム CUDA の基盤、環境変数、プラットフォームオーバーレイ、分離された serving 環境、そしてタスクが含まれています。
...
[pypi.indexes]                            # 依存関係が固定できる名前付きインデックス
...
chefe tree                 # 宣言済み vs インストール済み、それぞれのエコシステムでチェック
```
...
また、使い捨てのツールについては、manifest は不要です：
