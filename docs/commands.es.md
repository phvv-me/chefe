...
| comando | qué hace |
|---|---|
| `chefe init` | crea una estructura inicial de `chefe.toml` en el directorio actual |
...
| `chefe tree [env]` | compara lo declarado frente a lo instalado, verificando cada dependencia en **su propio** ecosistema |
| `chefe run <task> [args…]` | ejecuta una tarea dentro del entorno |
| `chefe x <cmd…>` | ejecuta un comando en un entorno desechable, similar a uvx o pipx run |
...
Cada paquete declarado se verifica contra el ecosistema en el que fue declarado. Conda se verifica contra el entorno de pixi, npm contra `.chefe/node_modules` y cargo contra el archivo `.crates.toml` del entorno. chefe reporta cada uno como `✓` ok, `≠` desviación o `✗` faltante, con un conteo transitivo.
...
chefe x --with build python -m build   # agrega paquetes adicionales con --with
```
