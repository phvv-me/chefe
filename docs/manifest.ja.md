...
単独の `[deps]` テーブルはデフォルトのソースである **conda** です。文字列のみの場合はバージョン指定となり、`*` は任意のバージョンを意味します。
...
!!! tip "ランタイムは自動的に確保されます"
    各エコシステムはインストールと実行のために言語ランタイムを必要とするため、不足している場合は chefe が conda-forge から追加します。`[pypi.deps]` は `python` を、`[npm.deps]` は `nodejs` を、`[cargo.deps]` は `rust` を、`[gem.deps]` は `ruby` をそれぞれ確保します。`[deps]` で独自の `python`（またはその他）を固定して宣言すれば、chefe はそれに干渉しません。これは環境ごとにも適用されるため、`[pypi.deps]` を使用する `no-default` 環境であっても、独自の `python` が確保されます。

## システム要件

クロスプラットフォーム解決のために使用される conda 仮想パッケージの最小要件であり、モジュールのロードではありません。
...
環境がアクティブ化された際に適用される静的な変数です。`.env` もデフォルトで読み込まれますが、ヘッダーの `dotenv` で切り替え可能です。
...
プラットフォームごとに条件付きで依存関係を追加します。これらはネイティブな pixi ターゲットにコンパイルされます。任意のスコープは `[on.…]` の下にネストします。
...
`chefe install serving` または `chefe tree serving` でインストールや確認が可能です。
