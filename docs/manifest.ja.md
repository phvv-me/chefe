...
単独の `[deps]` テーブルはデフォルトのソースである **conda** です。文字列のみの場合はバージョン指定を意味し、`*` は任意のバージョンを指します。
...
その他のエコシステムはすべて明示的に指定する必要があり、自動的に推論されることはありません。
...
環境がアクティブ化された際に適用される静的な変数です。`.env` もデフォルトで読み込まれます（ヘッダーの `dotenv` で切り替え可能）。
