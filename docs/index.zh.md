...
Conda、PyPI、npm、cargo。真正的项目往往需要同时使用多种工具，分散在 `pixi.toml`、`package.json` 和 `Cargo.toml` 中。chefe 是主厨。你只需编写**一份 `chefe.toml`** 配方，它就会将每个原生 manifest 编译到 `.chefe/` 下，运行真正的工具，并将它们整合为一个单一的环境。它从不重新实现解析器，而是指挥各个“厨师”工作。

<div class="grid cards" markdown>

- :material-silverware-variant: **一份配方**

    所有生态系统尽在单一 `chefe.toml` 中。无需再同时处理四个 manifest。

- :material-cog-transfer-outline: **原生输出**

    编译为真实的 `pixi.toml`、`package.json` 等文件。由实际的工具负责解析依赖。

- :material-source-branch: **可组合**

    平台覆盖和命名环境可以像 pixi 的特性（features）一样叠加。

- :material-broom: **自包含**

    整个环境都存在于 `.chefe/` 中，因此一条命令即可将其清除。
...
flowchart TB
    ...
    ENV(["一个已激活的环境<br/>.chefe/ 前缀在 PATH 中"]):::brand
...
- **结构**由 chefe 的 schema 进行验证，而**包规范**则保留为各工具的职责。
...
主厨从不独自烹饪每一道菜。他们编写配方并管理生产线，由厨师们各自负责自己的岗位。分散的包管理器就是那条生产线，而 chefe 则通过一份配方指挥它们。🧑‍🍳