...
![chefe — um manifest para cada gerenciador de pacotes](assets/banner.svg){ .hero-banner }
...
Conda, PyPI, npm, cargo. Projetos reais precisam de vários ao mesmo tempo, espalhados por `pixi.toml`, `package.json` e `Cargo.toml`. O chefe é o chef de cozinha. Você escreve **uma receita `chefe.toml`**, ele compila cada manifest nativo em `.chefe/`, executa as ferramentas reais e os serve como um único ambiente. Ele nunca reimplementa um resolvedor. Ele gerencia os cozinheiros.
...
Um chef de cozinha nunca prepara cada prato sozinho. Ele escreve a receita e comanda a linha, e os cozinheiros trabalham em suas respectivas estações. Gerenciadores de pacotes espalhados são essa linha, então o chefe os direciona a partir de uma única receita. 🧑‍🍳