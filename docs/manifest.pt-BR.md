...
A tabela simples `[deps]` é **conda**, a fonte padrão. Uma string simples é uma especificação de versão, e `*` significa qualquer uma.
...
!!! tip "Runtimes são garantidos automaticamente"
    Cada ecossistema precisa de seu runtime de linguagem para instalar e executar, então o chefe o adiciona a partir do conda-forge quando ausente. `[pypi.deps]` garante o `python`, `[npm.deps]` garante o `nodejs`, `[cargo.deps]` garante o `rust`, e `[gem.deps]` garante o `ruby`. Declare seu próprio `python` (ou outros) fixado em `[deps]` e o chefe não o alterará. Isso também se aplica por ambiente, portanto, um ambiente `no-default` que usa `[pypi.deps]` ainda obtém seu próprio `python`.
