...
| `chefe tree [env]` | 检查声明与已安装的包，每个依赖项都在**其各自的**生态系统中进行检查 |
...
Conda 是默认源，标志位可选择其他生态系统。`--spec` 设置版本（默认为 `*`），而 `--env` 则指向一个命名环境。
...
每个声明的包都会根据其声明所在的生态系统进行检查。Conda 针对 pixi 环境，npm 针对 `.chefe/node_modules`，cargo 针对环境的 `.crates.toml`。chefe 会报告每个包的状态为 `✓`（正常）、`≠`（偏差）或 `✗`（缺失），并附带传递依赖计数。
