...
Conda 是默认源，标志位用于选择其他生态系统。`--spec` 设置版本（默认为 `*`），而 `--env` 则指定目标环境。
...
每个已声明的包都会根据其声明所在的生态系统进行检查。Conda 对照 pixi 环境，npm 对照 `.chefe/node_modules`，cargo 对照环境的 `.crates.toml`。chefe 会报告每个包的状态：`✓` 表示正常，`≠` 表示偏差，`✗` 表示缺失，并附带传递依赖计数。
