...
| `chefe upgrade [pkg…]` | 現在の制約内で Pixi、Node、Cargo を更新し、指定したパッケージだけ制約を引き上げる |
...
| `chefe run <cmd> [args…]` | 環境内でタスクまたはインストール済み実行ファイルを実行する |
...
| `chefe global add <pkg…>` | workspace 名のグローバル環境へ conda パッケージを追加する |
| `chefe global remove <pkg…>` | workspace 名のグローバル環境からパッケージを削除する |
| `chefe global list` | すべてのグローバル環境、または `-e` で指定した環境を表示する |
...
`chefe run` はまず pixi に宣言済みタスクを解決させ、その後で管理環境の `PATH` にある通常の実行ファイルを探します。これにより、冗長なタスク alias を避けながら、アクティベーションスクリプトと環境変数を保ちます。
